...
> 主题：**性能修复**。修复 v0.4.2 主题重构引入的窗口拖动卡顿回归。

### 修复

- **玻璃主题拖动卡顿**：v0.4.2 的玻璃主题（mist 晨雾 / dusk 暮山）给整个窗口根容器额外叠加了一层 CSS `backdrop-filter: blur(16px)`，导致在桌面上高速拖动窗口时 WebView 每帧都要对整窗实时高斯模糊，出现明显延迟与卡顿。现移除该冗余的前端模糊层，玻璃模糊统一交由后端 DWM acrylic（由系统桌面合成器渲染，几乎零开销）提供——与 v0.4.1 mica 主题的流畅拖动行为一致，视觉观感基本无差异。
